...
+ (NSString *)generateLocalAudioFilePath:(NSString *)urlString
{
    ...
    BOOL isDir = FALSE;
    BOOL isDirExist = [fileManager fileExistsAtPath:AUDIO_CACHE_FOLDER isDirectory:&isDir];
    
    if(!(isDirExist && isDir))
    {
        BOOL bCreateDir = [fileManager createDirectoryAtPath:AUDIO_CACHE_FOLDER withIntermediateDirectories:YES attributes:nil error:nil];
        if(!bCreateDir) {
            NSLog(@"Create Audio Directory Failed.");
        }
    }
    
    NSString* localFilePath = [AUDIO_CACHE_FOLDER stringByAppendingPathComponent:[[StringUtils md5:urlString] lowercaseString]];
    
    if ([[urlString lowercaseString] hasSuffix:@".mp3"] || [[urlString lowercaseString] hasSuffix:@".mpga"]) {
...
{
    if ([_delegate respondsToSelector:@selector(visualDownloaderDidCancel)]) {
        [_delegate visualDownloaderDidCancel];
    }
