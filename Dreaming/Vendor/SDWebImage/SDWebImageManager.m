...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if ([imageCache imageFromKey:key fromDisk:NO])
    {
        if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImageFromCache:)]) {
            [delegate performSelector:@selector(webImageManager:didFinishWithImageFromCache:) 
                           withObject:self 
                           withObject:image];
        }
    }
